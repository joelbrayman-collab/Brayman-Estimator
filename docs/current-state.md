...
| **Working tree at last verified inspect** | **FG-021 IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT OPEN.** Gate **NOT CLOSED**. Bounded LAN iPhone Save Original UUID repair landed (`field.js` `crypto.getRandomValues` v4 fallback). Live current = head **`d2e3f4a5b6c7`**. Full suite **553 passed**. Dedicated FG-021 **15**. Focused **143**. Dedicated FG-020 **44**. [ADR-043](adr/ADR-043-field-web-capture-reliability-local-pending-and-idempotent-replay.md) **Accepted**. FG-020 **CLOSED / OPERATIONAL FOR UAT**. Real iPhone UAT **not complete**. Native Signing **DEVELOPMENT MAY PROCEED UNDER SEPARATE GOVERNANCE**; **PRODUCTION ACTIVATION BLOCKED PENDING COUNSEL PROCESS APPROVAL**. |
...
- **Field Web V1 Today + Capture (FG-021)** — **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT OPEN.** `/field` Today, Project confirm, Capture (photo/voice/text). IndexedDB `calibai-field-v1`. Idempotent Event/Original POST (201/200/409). Display GET for Field image preview. Additive revision **`d2e3f4a5b6c7` applied live** (`c1d2e3f4a5b6` → `d2e3f4a5b6c7`). Bounded LAN iPhone Save Original repair: `newUuid()` uses `crypto.randomUUID()` when available, else `crypto.getRandomValues` RFC 4122 v4 (no `Math.random()`). Live current = head. Gate **NOT CLOSED**. Real iPhone UAT **not complete**. No PWA. No transcription. Dedicated tests **15 passed**. Focused **143 passed**. Full suite **553 passed**.
...
- **Approved, next:** Retry real iPhone Safari UAT Save Original on the same HTTP LAN URL after a hard refresh. Do **not** close FG-021 until UAT passes. Native Signing **development** may proceed only under **separate** governance; **production activation** remains blocked pending counsel process approval. Do **not** implement Project Closeout or Contract templates. Do **not** weaken the Legal Content Gate.
...
1. **FG-021 IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT OPEN.** Gate **NOT CLOSED**. Live current = head `d2e3f4a5b6c7`. Retry Save Original on the same iPhone Safari UAT URL after a hard refresh. Do **not** close FG-021 until real iPhone Safari UAT passes. Do not begin Native Signing production, Closeout, MONITOR, transcription, PWA, or external AI. Do not accept ADR-008. Do not accept ADR-010.
