"""FG-024 Slice C contract generation + immutable snapshot foundation.

Reuses Slice A selector and ADR-037 via that selector. Does not own legal
approval, Native Signing, warranty-schedule generation, or ADR-051 §6 policy.
Pending-candidate generation is unimplemented / fail-closed.
"""
...
TEST_ELIGIBLE_VERSION_STATUSES = frozenset({"Issued", "Accepted"})
...
def _open_pending_review(package: LegalContentJurisdictionPackage) -> bool:
    """Unsupported generation branch. Not ADR-051 §6 ALLOW/WARN/BLOCK policy."""
    if package.support_status == "UPDATE_PENDING_REVIEW":
        return True
    direct = LegalContentCandidateChange.query.filter(
    ...
    impact = (
        LegalContentCandidateImpact.query.join(
            LegalContentCandidateChange,
            LegalContentCandidateImpact.candidate_id
            == LegalContentCandidateChange.id,
        )
        .filter(
...
def _authoritative_objects(package: LegalContentJurisdictionPackage):
    rows = [
        obj
        for obj in package.content_objects
        if obj.kind == REQUIRED_LEGAL_KIND
    ]
...
def generate_project_contract(
    ...
    *,
    ...
    if version.status not in TEST_ELIGIBLE_VERSION_STATUSES:
    ...
    selection = select_legal_content_package_for_project(project.id, as_of=as_of)
    if not selection.available or selection.status != STATUS_AVAILABLE:
    ...
    if _open_pending_review(package):
        return _block(BLOCK_PENDING_REVIEW_UNSUPPORTED)

    objects = _authoritative_objects(package)
    if not objects:
