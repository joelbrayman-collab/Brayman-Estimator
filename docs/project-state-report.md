...
| Current branch | `main` |
| Current commit / `origin/main` | This Material Catalogue architecture-docs commit (verify `git log -1` after push). Started from `450cd39dea24c3e41d32defa39e9e74c00ae7c6d`. FG-013 product: `974136bb2ac7d2f61acf71b53f81a2ae55f132b1`. FG-012: `0b403d6aa51381d3763cf3dc9d5d96e096d5ab93`. |
...
| Architecture status | CAR-001 approved. FG-008 / FG-009 / FG-010 / FG-011 / FG-012 / **FG-013 CLOSED / OPERATIONAL FOR UAT**. Material Catalogue architecture **documented** (not implemented). ADR-019 **Accepted**. **ADR-021 Accepted**. **ADR-032 Accepted**. **ADR-033 Accepted** (supplier channel; not implemented). ADR-005/006/007/009/011/031 **Accepted**. ADR-008 / ADR-010 **Proposed**. Real external AI provider **not authorized**. Phase D **not started**. Supplier integration **not started**. |
...
| Incomplete work | Material Catalogue ADRs then Feature Gate; Phase D estimate mapping; four-output outputs 3–4; QuickBooks; Ontario contract/warranty; BUILD field capture; MONITOR implementation; office authentication; industry benchmarking; supplier/Winchester POC. |
...
| Documentation status | FG-013 **CLOSED / OPERATIONAL FOR UAT**. Material Catalogue architecture **documented** (not implemented). ADR-032 **Accepted**. **ADR-033 Accepted** (supplier channel; docs only). ADR-021 **Accepted** (docs only). FG-012 **CLOSED / OPERATIONAL FOR UAT**. ADR-008 / ADR-010 **Proposed**. |
| Decisions made (this implementation pass) | Material Catalogue: CalibAi-seeded identity; CostItem remains org costing; living supplier evidence (incl. price increases and promotions) is not the identity row; first FG identity-only; Phase D after identity; MaterialRequirement future; Material Cost Standard deferred; ADR-008 not accepted here. |
| Decisions pending | Material Catalogue **ADRs**. Real AI provider; Phase D mapping gate. FG-009 carry-forward unchanged. Darcy commercial terms unset. Supplier Feature Gate **not authorized**. |
| Uncommitted work | None expected after this docs commit/push. |
| Next approved milestone | **Material Catalogue ADRs** (docs) when separately authorized. Do not implement. Do not open a Feature Gate yet. Do not `flask db upgrade`. Do not implement MONITOR. Do not start Phase D. Do not start supplier/Winchester POC. |
| Next candidate milestone | Material Catalogue ADRs, then Feature Gate (lumber/sheets identity). Phase D mapping remains **NOT STARTED / NOT AUTHORIZED**. MONITOR implementation remains **not authorized**. |
| Documents to read first | [current-state.md](current-state.md) → [session-handoff.md](session-handoff.md) → [architecture/material-catalogue-architecture.md](architecture/material-catalogue-architecture.md) → [adr/ADR-033-supplier-neutrality-and-launch-partner-channel.md](adr/ADR-033-supplier-neutrality-and-launch-partner-channel.md) |
| Approved next Cursor prompt location or summary | **Material Catalogue ADRs** (docs only). Do not implement Material Catalogue. Do not create a Feature Gate. Do not accept ADR-008 unless that prompt authorizes it. Do not implement MONITOR. Do not start Phase D. Do not enable external AI. Do not start supplier / Winchester POC. Do not `flask db upgrade`. |
| Commit status | Material Catalogue **architecture docs** this commit. Graph head / live current `c5d6e7f8a9b0`. |
