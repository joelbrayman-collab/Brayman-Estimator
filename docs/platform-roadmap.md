...
1. **STOP.** [FG-014](feature-gates/FG-014-material-catalogue-v1-dimensional-lumber-sheet-goods.md) **APPROVED FOR IMPLEMENTATION / IMPLEMENTATION NOT STARTED**. Next: **bounded FG-014 implementation prompt** when separately authorized. Do **not** implement in this governance pass. Do **not** `flask db upgrade`. Do not implement MONITOR. Do not start Phase D. Do not start supplier/Winchester POC or bulk supplier onboarding. Do not accept ADR-008.
...
1. Authorize the **FG-014 implementation prompt** (identity-only lumber/sheets; one additive migration when that prompt says so). Do **not** implement supplier integration, bulk supplier onboarding, or a Winchester POC. Do **not** `flask db upgrade` until a live-migrate prompt. Do not accept ADR-008.
