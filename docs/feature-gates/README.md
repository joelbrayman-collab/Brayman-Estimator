...
| [FG-005](FG-005-m010-scale-calibration.md) | M010 Scale Calibration / Measurement Tools | **APPROVED** — implementation **not started** (Milestone 010) |

Implementation is not authorized by a Feature Gate until Joel approves the gate and the corresponding Cursor prompt.

**CAR-001** aligns strategic CalibAi lifecycle architecture. **FG-004** authorized M009, which is now implemented and verified in code on `main`. **FG-005** authorizes M010 scope/architecture; implementation requires a dedicated implementation prompt.
