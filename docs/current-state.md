...
|-------|--------|
| Branch | `main` |
| HEAD / `origin/main` | FG-020 Compatible Rendition increment: verify `git rev-parse HEAD` and `git rev-parse origin/main` after this commit. Parent `77d496367f9e6f003eb69949adb3bd82c6cadfd7`. Live Alembic current **`b0c1d2e3f4a5`**. Repository Alembic head **`c1d2e3f4a5b6`**. |
...
- **Project Hub UX (FG-011)** — **IMPLEMENTED / VERIFIED**. `/projects/<id>` is the office-estimator Project Hub: PLAN / PRICE / CONTRACT stored facts and links; BUILD = existing Change Orders **plus** Field Observations ([FG-020](feature-gates/FG-020-build-field-capture-v1-project-field-observation-foundation.md) **IMPLEMENTED / LIVE MIGRATION PENDING**); MONITOR / LEARN labeled Future. Read-only `app/services/project_hub.py`.
...
- **BUILD Field Capture V1 (FG-020)** — **IMPLEMENTED / LIVE MIGRATION PENDING.** Field Capture Events, Original Payloads (text/audio/image including HEIC/HEIF originals without transcode), Derived Candidates, private custody `instance/build_originals/`, automatic HEIC/HEIF → JPEG Compatible Renditions at `instance/build_renditions/`, office Field Observations beside Change Orders, bounded `/api/v1` BUILD routes, `flask build propose-derived-candidate`. Additive revision **`c1d2e3f4a5b6`** is the repository head; live current remains **`b0c1d2e3f4a5`**. Dedicated tests **44 passed** (33 + 11). Full suite **538 passed**. Not closed. Field Web **not** implemented. Live `flask db upgrade` **not** run. Project Closeout **not** started.
...
- **Organization Brand Profile** ([architecture/organization-brand-profile.md](architecture/organization-brand-profile.md)) is **CLOSED / OPERATIONAL FOR UAT**. [ADR-040](adr/ADR-040-organization-brand-profile.md) **Accepted**. [FG-017](feature-gates/FG-017-organization-brand-profile-v1.md). **Gate-at-close** current was `a9b0c1d2e3f4`; live head today is `b0c1d2e3f4a5`. Change Order / Permit consumers remain future.
...
- Live development/UAT `flask db current`: `b0c1d2e3f4a5` (FG-018). FG-020 upgrade **pending**. One graph head.

## Current milestone status

M005–M011, **FG-006**, **FG-008**, **FG-009**, and **M012 / FG-010** remain **implemented, verified, committed, and pushed** on `main`.

- **Current coded work:** [FG-020](feature-gates/FG-020-build-field-capture-v1-project-field-observation-foundation.md) **IMPLEMENTED / LIVE MIGRATION PENDING**. [FG-019](feature-gates/FG-019-shared-api-foundation-v1.md) **CLOSED / OPERATIONAL FOR UAT**. [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) **CLOSED / OPERATIONAL FOR UAT**. [FG-017](feature-gates/FG-017-organization-brand-profile-v1.md) **CLOSED / OPERATIONAL FOR UAT**. [FG-016](feature-gates/FG-016-ontario-ottawa-permit-intelligence-poc.md) **CLOSED / OPERATIONAL FOR UAT**.
- **Approved, next:** Give Ontario counsel [legal/native-signing-process-counsel-review.md](legal/native-signing-process-counsel-review.md). Do **not** implement Native Signing. Do **not** create a Feature Gate. FG-020 close remains a separate live-migration / office UAT prompt (`b0c1d2e3f4a5` → `c1d2e3f4a5b6`). Do **not** mark FG-020 closed. Do **not** implement Project Closeout or Field Web. Do **not** start Contract implementation.
- **Roadmap direction (not authorization):** item 10 is **COMPLETE**. Item 11 is implemented / live migration pending (including image-only Compatible Renditions). Project Closeout **FUTURE**. **ROADMAP SEQUENCE ≠ IMPLEMENTATION AUTHORIZATION.**
- **Blocked / Not Started (product):** Project Closeout / archive-and-purge; Field Web; Phase D estimate mapping; four-output package outputs 3–4; QuickBooks; contracts; MONITOR implementation; LEARN; industry benchmarking; historical evidence repair; real external AI provider; supplier/Winchester POC; bulk supplier onboarding; national Permit Rules expansion; Change Order document family; Permit branding; RBAC; org-switcher.
...
1. **STOP after this increment.** [FG-020](feature-gates/FG-020-build-field-capture-v1-project-field-observation-foundation.md) is **IMPLEMENTED / LIVE MIGRATION PENDING**. Image-only Compatible Renditions are in. Live current remains `b0c1d2e3f4a5`. Repository head is `c1d2e3f4a5b6`. Do **not** run live `flask db upgrade` until a separate live-migration prompt. Do **not** mark FG-020 **CLOSED / OPERATIONAL FOR UAT**. Do **not** start Field Web. Do **not** implement Project Closeout. Do not begin national permit expansion, Phase D, Change Order document work, supplier integration, or external AI / runtime web. Do not accept ADR-008. Do not accept ADR-010.
2. **Next authorized prompt:** live-migrate `b0c1d2e3f4a5` → `c1d2e3f4a5b6`, then office UAT of Field Observations beside Change Orders, including HEIC photos displaying as JPEG. Item 12 remains **BLOCKED / NOT AUTHORIZED**.
