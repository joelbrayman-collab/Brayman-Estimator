...
|-------|--------|
| Branch | `main` |
| HEAD / `origin/main` | Verify `git log -1` after this docs commit. Product implementation: FG-013 `974136bb2ac7d2f61acf71b53f81a2ae55f132b1`. Material Catalogue architecture `b53d9e7150e43b173bad3c26eee8e829529773e5`. Alembic **current = head = `c5d6e7f8a9b0`**. |
...
1. **STOP.** Material Catalogue **ADRs Accepted** (034–036). Next: **Material Catalogue Feature Gate** when separately authorized (identity-only lumber/sheets). Do **not** implement until that gate. Do **not** run `flask db upgrade`. Do not implement MONITOR, Phase D, supplier integration, or Winchester POC. Do not accept ADR-008 in the identity pass.
