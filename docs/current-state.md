...
|-------|--------|
| Branch | `main` |
| HEAD / `origin/main` | FG-021 implementation: verify `git rev-parse HEAD` and `git rev-parse origin/main` after this commit. Parent `eb4466c4d090aaf366d39ebe2e3ff8ec1a382993`. Live Alembic current **`c1d2e3f4a5b6`**. Graph head **`d2e3f4a5b6c7`**. |
...
| **Working tree at last verified inspect** | **FG-021 IMPLEMENTED / LIVE MIGRATION PENDING.** Gate **NOT CLOSED**. Revision `d2e3f4a5b6c7` created. Live current remains `c1d2e3f4a5b6`. Full suite **551 passed**. Dedicated FG-021 **13**. Focused **141**. Dedicated FG-020 **44**. [ADR-043](adr/ADR-043-field-web-capture-reliability-local-pending-and-idempotent-replay.md) **Accepted**. FG-020 **CLOSED / OPERATIONAL FOR UAT**. Real iPhone UAT **not complete**. Native Signing **DEVELOPMENT MAY PROCEED UNDER SEPARATE GOVERNANCE**; **PRODUCTION ACTIVATION BLOCKED PENDING COUNSEL PROCESS APPROVAL**. |
...
- **Field Web V1 Today + Capture (FG-021)** — **IMPLEMENTED / LIVE MIGRATION PENDING.** `/field` Today, Project confirm, Capture (photo/voice/text). IndexedDB `calibai-field-v1`. Idempotent Event/Original POST (201/200/409). Display GET for Field image preview. Revision **`d2e3f4a5b6c7`** created; live current remains **`c1d2e3f4a5b6`**. Gate **NOT CLOSED**. Real iPhone UAT **not complete**. No PWA. No transcription. Dedicated tests **13 passed**. Full suite **551 passed**.
...
- **Organization Brand Profile** ([architecture/organization-brand-profile.md](architecture/organization-brand-profile.md)) is **CLOSED / OPERATIONAL FOR UAT**. [ADR-040](adr/ADR-040-organization-brand-profile.md) **Accepted**. [FG-017](feature-gates/FG-017-organization-brand-profile-v1.md). **Gate-at-close** current was `a9b0c1d2e3f4`; live head today is `c1d2e3f4a5b6`. Change Order / Permit consumers remain future.
...
1. **FG-021 IMPLEMENTED / LIVE MIGRATION PENDING.** Gate **NOT CLOSED**. Live current remains `c1d2e3f4a5b6`. Do **not** run live `flask db upgrade` except under an explicit live-migration prompt. Do **not** close FG-021 until real iPhone UAT passes. Do not begin Native Signing production, Closeout, MONITOR, transcription, PWA, or external AI. Do not accept ADR-008. Do not accept ADR-010.
