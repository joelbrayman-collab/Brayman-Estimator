...
| Layer | Position |
|-------|----------|
| **CURRENT** | FG-008 through FG-018 **CLOSED / OPERATIONAL FOR UAT** as applicable. Live current = head **`b0c1d2e3f4a5`**. Full suite **460 passed**. Office auth UAT port **5011**. |
| **NEXT AUTHORIZED ACTION** | **STOP.** Do not start shared API, BUILD, RBAC, or org-switcher. Wait for a separately authorized next Feature Gate. |
| **ROADMAP DIRECTION** | Item 10 office-auth is **CLOSED / OPERATIONAL FOR UAT**. Shared API **deferred**. Items **11–12** require a later shared-API/BUILD gate. **ROADMAP SEQUENCE ≠ IMPLEMENTATION AUTHORIZATION.** |
...
- **`main` / `origin/main`** — FG-018 implementation `0d7af3e93a9d6c4f27eb2136f915297620be59ed`. Live `HEAD` / `origin/main`: verify `git rev-parse HEAD` and `git rev-parse origin/main` after the close commit. Alembic **current = head = `b0c1d2e3f4a5`**. One graph head.
...
10. Authentication / actor identity + shared API foundation — **office slice CLOSED / OPERATIONAL FOR UAT.** [ADR-041](adr/ADR-041-user-membership-and-office-authentication.md) **Accepted**. [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) **CLOSED / OPERATIONAL FOR UAT** (office User + membership + login). Live current = head `b0c1d2e3f4a5`. Shared API **deferred** to a later gate.
...
**ROADMAP SEQUENCE ≠ IMPLEMENTATION AUTHORIZATION.** [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) **CLOSED / OPERATIONAL FOR UAT** does **not** authorize shared API, BUILD, RBAC, or org-switcher.
...
2. **Roadmap direction:** item 10 office-auth slice is [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) (**CLOSED / OPERATIONAL FOR UAT**). Shared API remains a later gate. Items 11–12 remain dependent on a later shared-API/BUILD gate.
...
- Authentication / actor identity — [ADR-041](adr/ADR-041-user-membership-and-office-authentication.md) **Accepted**; [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) **CLOSED / OPERATIONAL FOR UAT** (office login/membership operational for UAT). Shared API **deferred**.
- BUILD field capture; field web (sequence items 11–12; **require a later shared-API/BUILD gate**)
