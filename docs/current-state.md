...
|-------|--------|
| Branch | `main` |
| HEAD / `origin/main` | Verify `git log -1` after this docs commit. Product implementation: FG-013 `974136bb2ac7d2f61acf71b53f81a2ae55f132b1`. Material Catalogue ADRs `130b3fd35114014f0635d9a70e7cb3096647d480`. Alembic **current = head = `c5d6e7f8a9b0`**. |
...
- **Material Catalogue** — [architecture/material-catalogue-architecture.md](architecture/material-catalogue-architecture.md) **Intended**; [FG-014](feature-gates/FG-014-material-catalogue-v1-dimensional-lumber-sheet-goods.md) **APPROVED FOR IMPLEMENTATION / IMPLEMENTATION NOT STARTED**; [ADR-034](adr/ADR-034-canonical-material-identity-and-ownership.md) / [ADR-035](adr/ADR-035-material-quantity-uom-and-requirement-boundary.md) / [ADR-036](adr/ADR-036-material-commercial-evidence-and-supplier-mapping.md) **Accepted** (**not implemented**; ADR-008 remains Proposed)
...
- Live development/UAT `flask db current`: `c5d6e7f8a9b0` (**VERIFIED APPLIED**; not upgraded by the reconciliation pass)

## Current milestone status

M005–M011, **FG-006**, **FG-008**, **FG-009**, and **M012 / FG-010** remain **implemented, verified, committed, and pushed** on `main`.

- **Current coded work:** [FG-013](feature-gates/FG-013-contractor-calibration-onboarding-historical-upload-ux.md) **CLOSED / OPERATIONAL FOR UAT**. [FG-014](feature-gates/FG-014-material-catalogue-v1-dimensional-lumber-sheet-goods.md) **APPROVED FOR IMPLEMENTATION / IMPLEMENTATION NOT STARTED** (docs only). [Material Catalogue architecture](architecture/material-catalogue-architecture.md) **Intended**. ADR-032 **Accepted**. ADR-033 **Accepted** (docs only). ADR-021 **Accepted** (docs only). FG-012 remains **CLOSED / OPERATIONAL FOR UAT.**
- **Blocked / Not Started (product):** FG-014 **implementation**; Phase D estimate mapping; four-output package outputs 3–4; QuickBooks; contracts; BUILD field capture; MONITOR implementation; LEARN; industry benchmarking; historical evidence repair; real external AI provider; office authentication; supplier/Winchester POC; bulk supplier onboarding.
...
1. **STOP.** [FG-014](feature-gates/FG-014-material-catalogue-v1-dimensional-lumber-sheet-goods.md) **APPROVED FOR IMPLEMENTATION / IMPLEMENTATION NOT STARTED**. Next: **bounded FG-014 implementation prompt** when separately authorized (identity-only lumber/sheets). Do **not** implement in this governance pass. Do **not** run `flask db upgrade`. Do not implement MONITOR, Phase D, supplier integration, bulk supplier onboarding, or Winchester POC. Do not accept ADR-008.
