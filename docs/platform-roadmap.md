...
| **NEXT AUTHORIZED ACTION** | **STOP product implementation.** [ADR-041](adr/ADR-041-user-membership-and-office-authentication.md) **Proposed**. [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) **DRAFT / NOT APPROVED**. No implementation prompt authorized. |
| **ROADMAP DIRECTION** | Item 10 **governance has begun**. Office-auth slice is FG-018 (**draft only**). Shared API **deferred**. Authentication **implementation NOT STARTED**. Items **11–12 require Item 10 implementation**. **ROADMAP SEQUENCE ≠ IMPLEMENTATION AUTHORIZATION.** |
...
- **`main` / `origin/main`** — FG-017 close `620dec1a9612e87a1ede20cfa6aa46c6d72a8dd5`. Docs-reconciliation content `dd30d752190e56ed687e270950df9bf9a06d7a26`. SHA-pin `07cb46c501d968542dff567943044dc1db870f01`. Live `HEAD` / `origin/main`: verify `git rev-parse HEAD` and `git rev-parse origin/main` (do not treat as a circular this-commit reference). Alembic **current = head = `a9b0c1d2e3f4`**. One graph head.
...
**ROADMAP SEQUENCE ≠ IMPLEMENTATION AUTHORIZATION.** [ADR-041](adr/ADR-041-user-membership-and-office-authentication.md) **Proposed** and [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) **DRAFT / NOT APPROVED** do **not** authorize product implementation.
...
2. **Roadmap direction:** item 10 office-auth slice is [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) (**draft only**). Items 11–12 remain dependent on Item 10 **implementation**, not on this draft.
...
- Authentication / actor identity — [ADR-041](adr/ADR-041-user-membership-and-office-authentication.md) **Proposed**; [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) **DRAFT / NOT APPROVED** (office login/membership; **not implemented**). Shared API **deferred**.
...
1. [FG-017](feature-gates/FG-017-organization-brand-profile-v1.md) is **CLOSED / OPERATIONAL FOR UAT**. **STOP product implementation.** [ADR-041](adr/ADR-041-user-membership-and-office-authentication.md) is **Proposed**. [FG-018](feature-gates/FG-018-organization-authentication-actor-identity-and-membership-v1.md) is **DRAFT / NOT APPROVED**. Do **not** implement Authentication, national permit expansion, Phase D, Change Order document work, supplier integration, bulk supplier onboarding, or a Winchester POC. Do not accept ADR-008.
2. Whether to **Accept ADR-041** and **Approve FG-018** for a later implementation reconnaissance (not this pass). Shared API remains a **later gate**. Items 11–12 remain blocked until Item 10 is implemented.
