...
- Project statuses include default `Lead` (model default). `Project.status` is the CRM / pipeline label and is **not** operating-set authority. CORE CLOSE Slice A persists `Project.operating_state` **ACTIVE / CLOSED** plus `ProjectOperatingStateEvent` and `list_current_operating_projects` in the working tree ([architecture/core-close-project-lifecycle-product-direction.md](../architecture/core-close-project-lifecycle-product-direction.md)). Live DB is **not** migrated. Live consumers still use `list_organization_projects`. Close / Reopen / Punch List / Completion Sign-Off remain **NOT IMPLEMENTED**.
