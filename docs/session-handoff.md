...
**Last authorized delta:** **2026-09-17 FG-035 PERF-C Slice A implementation (working tree).** PERF-C **IMPLEMENTED IN WORKING TREE / TESTED / NOT COMMITTED / NOT PUSHED / NOT LIVE-UATed**. Office `/company-attention`. Requires `COMPANY_MANAGEMENT`. No schema. No live UAT data. PERF-A / PERF-B **SEALED**. FG-037 **CLOSED**. People & Access unchanged. Scores unchanged. Recovery stash **not dropped**. Next action **STOP.** Return to ChatGPT Architect. Do **not** commit. Do **not** create live UAT data. Do **not** implement People & Access. Do **not** drop the recovery stash.
