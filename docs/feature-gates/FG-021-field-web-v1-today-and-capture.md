...
| Approved baseline | Live Alembic current = head **`d2e3f4a5b6c7`**. Latest product-changing governed suite: dedicated FG-021 **20**; focused **148**; full **558**. Dedicated FG-020 **44**. |
...
**Current remaining real-iPhone UAT (2026-09-06 after current-iPhone field-usability PASS):** session-expiry recovery (**OPEN / DEFERRED / NOT YET EXERCISED**); older supported iPhone/Safari smoke where practical. Portrait / one-handed / outdoor / landscape / orientation are **PASS**. Current-iPhone field-usability package is **PASS**. Observation Delete remains **QUEUED / NOT AUTHORIZED / NOT IMPLEMENTED**. CSRF recovery is **PASS**. Do **not** close FG-021 from this PASS alone.
...
**Subsequent status (2026-09-06 landscape-tolerance real-iPhone UAT):** **PASS.** Implemented landscape CSS at `057ff15`. Initial post-fix retest **FAIL** was confounded by iPhone Portrait Orientation Lock **ON** (not a CalibAi product defect). After Joel turned the lock **OFF**, Field adjusted correctly on physical landscape rotation. Portrait **PASS**. One-handed **PASS**. Outdoor readability **PASS**. **ORIENTATION / PORTRAIT PASS.** **CURRENT-IPHONE FIELD-USABILITY PASS.** No second CSS correction. Gate **NOT CLOSED**.

**Subsequent status (2026-09-06 landscape-tolerance CSS correction):** **DEFECT CORRECTED IN CODE / REAL-IPHONE RETEST REQUIRED / NOT YET PASS.** Operator FAIL: phone does not adjust for landscape. Portrait **PASS**. One-handed **PASS**. Outdoor readability **PASS**. General visibility **PASS**. CSS-only `@media (orientation: landscape)` in `app/static/css/field.css` after the existing `min-width: 720px` rule. Portrait defaults unchanged. Templates/JS unchanged. Dedicated **20**. Focused **148**. Full **558**. Gate **NOT CLOSED**.

**Subsequent status (2026-09-06 CSRF recovery real-iPhone UAT):** **PASS.** Natural Flask-WTF expiry on the same Capture document. Authoritative T0 `GET /field/projects/11/capture` **200** `06:02:58` EDT. Conservative Save time `07:03:58` EDT. Save `07:40:34` EDT (dwell **5856 s**). Detached HTTPS PID **88819** `https://192.168.134.223:5443`, **Project 11**, text-only. Sequence: Event POST **400** → Capture GET **200** (no static; `refreshCsrf` fetch) → Event POST **201** → Original POST **201**. No login/logout/401. Event **39** / Original **39**, `ORG-001`, Joel Brayman `user_id` 1. `kind=text` body `FG-021 CSRF recovery UAT - 6 Sep 2026` (ASCII hyphen; operator instruction used an em dash — not a product defect). `client_capture_uuid` `0012e6af-c627-4c40-9793-5e1e6611691f`; `client_original_uuid` `acba9f83-5ed2-40ae-a811-c805318594c0`. Live **39** / **39** (+1/+1). First 400 created no Event. One Event + one text Original; UUID counts 1/1; no orphan. Operator UI **SAVED**. Event **38** remains interrupted-CSRF pending recovery residue (not this PASS). SESSION-EXPIRY RECOVERY remains **OPEN / DEFERRED / NOT YET EXERCISED**. Gate **NOT CLOSED**.

**Subsequent status (2026-09-05 background/foreground persistence real-iPhone UAT):** **PASS.** Unsaved Capture text survived ordinary Safari backgrounding (not force-close) and remained usable after foreground. HTTPS `https://192.168.134.223:5443`, **Project 11**, text-only. Flask Cursor Terminal **556723**: Capture GET `07:17:25`; no Event POST until `07:19:10` Event **201** then Original **201**. Today GET **200** `07:19:17`. Event **36** / Original **36**, `ORG-001`, Joel Brayman `user_id` 1. `kind=text` body `FG-021 background-foreground UAT - 5 Sep 2026` (ASCII hyphen; operator instruction used an em dash — not a product defect). `client_capture_uuid` `3394825c-bb74-409b-93c6-d976021ec39e`; `client_original_uuid` `f4ede486-1b7b-40de-be02-47e5af04c5ba`. Live **36** / **36** (+1/+1). Desktop Hub `/projects/11/field-events/36` **200**. One Event + one text Original; no 409. Gate **NOT CLOSED**.

**Subsequent status (2026-09-05 mixed-capture real-iPhone UAT):** **PASS.** One Save: short text + Take Photo JPEG + one voice clip. HTTPS `https://192.168.134.223:5443`, **Project 11**. Flask Cursor Terminal **556723** `06:52:40` Event POST **201** then three Original POSTs **201**. Event **35**, `ORG-001`, Joel Brayman `user_id` 1. `client_capture_uuid` `addef41b-5120-4361-b2c7-b99e4b9c7711`. Original **33** `kind=text` body `FG-021 mixed capture UAT - 5 Sep 2026` (ASCII hyphen; operator instruction used an em dash — not a product defect). Original **34** `kind=audio` `note.m4a` `audio/mp4` 190338 bytes SHA-256 `92d3896b7f88887dd33bd3dc443609a413e8581a8bb86d2b743de6258e5b6c5f` stored `instance/build_originals/ORG-001/11/35/34.m4a`. Original **35** `kind=image` `image.jpg` JPEG JFIF 3076035 bytes SHA-256 `d3e947e6603e7d1dbd947634e0a603778de2e2688367266e3972f3d61f9c3bae` stored `instance/build_originals/ORG-001/11/35/35.jpg`. Kind set exactly `{text, image, audio}`. Live **35** / **35** (+1 Event / +3 Originals). Field Today GET **200** and image display **200**. Desktop Hub `/projects/11/field-events/35` **200**. One Event; no duplicate/orphan Originals. Gate **NOT CLOSED**.
