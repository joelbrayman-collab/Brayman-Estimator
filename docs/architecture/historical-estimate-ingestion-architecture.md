...
**Subsequent custody (2026-08-30 — [ADR-032](../adr/ADR-032-app-managed-historical-workbook-storage.md) Accepted; [FG-013](../feature-gates/FG-013-contractor-calibration-onboarding-historical-upload-ux.md) CLOSED / OPERATIONAL FOR UAT):**
...
- **Productized FG-013 uploads (coded):** `instance/historical_uploads/<organization_id>/<sha256>.xlsx|.xlsm` via `app/services/historical_ingestion/storage.py`. Durable rows: `historical_upload_attempts`. Office UX: `POST /historical-estimates/upload`. Live table **verified applied** (`c5d6e7f8a9b0`).
