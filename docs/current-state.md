...
|-------|--------|
| Branch | `main` |
| HEAD / `origin/main` | Verify `git log -1` after this docs commit. Product: FG-014 **LIVE-MIGRATED / FLASH REPAIR APPLIED — OFFICE RE-UAT REMAINING**. Alembic **current = head = `d6e7f8a9b0c1`**. Implementation `976cc4a4942ae346b9843a77126f89969bba2b6e`. |
...
- **Material Catalogue V1 (FG-014)** — **LIVE-MIGRATED / FLASH REPAIR APPLIED — OFFICE RE-UAT REMAINING.** Platform `canonical_materials` (27 lumber/sheet seed rows) **applied live**; optional Material `CostItem.canonical_material_id`; office `/material-catalogue/`. Live current = head `d6e7f8a9b0c1`. Catalogue-link flash repaired: `link_cost_item` now catches `MaterialCatalogueError` before `(TypeError, ValueError)`. Dedicated tests **35 passed**. Full suite **345 passed**. Prior office UAT on port **5005** passed list/search/filter/detail/Material link-unlink/isolation GET 404/assembly read-through. Live repaired POST on port **5006** flashed `Labour cost items cannot link to a canonical material.` for `FG014-UAT-LAB`. Short office re-UAT remains before gate close. No supplier pricing/SKU/inventory.
...
- **Project Permit & Approvals Report** ([architecture/permit-and-approvals-report.md](architecture/permit-and-approvals-report.md)) is **FUTURE / NOT IMPLEMENTED**. Advisory preflight pin only. Does not authorize Permit Intelligence, jurisdictional legal-library, live regulatory AI, in-product web lookup, automatic approval conclusions, municipal submissions, schema, ADR, or a Feature Gate. Does not interrupt FG-014.
...
- **Current coded work:** [FG-014](feature-gates/FG-014-material-catalogue-v1-dimensional-lumber-sheet-goods.md) **LIVE-MIGRATED / FLASH REPAIR APPLIED — OFFICE RE-UAT REMAINING**. [FG-013](feature-gates/FG-013-contractor-calibration-onboarding-historical-upload-ux.md) **CLOSED / OPERATIONAL FOR UAT**.
- **Blocked / Not Started (product):** FG-014 **office re-UAT of catalogue-link flashes then close**; Phase D estimate mapping; four-output package outputs 3–4; QuickBooks; contracts; BUILD field capture; MONITOR implementation; LEARN; industry benchmarking; historical evidence repair; real external AI provider; office authentication; supplier/Winchester POC; bulk supplier onboarding; Permit & Approvals Report.
...
- **Permit & Approvals Report pin** (FUTURE / NOT IMPLEMENTED; Mike Pratt Coach House at 2562 Church Street, North Gower, Ontario as future architecture/UAT reference) — [architecture/permit-and-approvals-report.md](architecture/permit-and-approvals-report.md)
...
1. **STOP.** [FG-014](feature-gates/FG-014-material-catalogue-v1-dimensional-lumber-sheet-goods.md) **LIVE-MIGRATED / FLASH REPAIR APPLIED — OFFICE RE-UAT REMAINING**. Next: **office re-UAT of catalogue-link error flashes, then close FG-014**. Do **not** run `flask db upgrade` again. Do not implement MONITOR, Phase D, supplier integration, bulk supplier onboarding, Winchester POC, or the Permit & Approvals Report. Do not accept ADR-008.
