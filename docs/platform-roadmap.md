...
| Layer | Position |
|-------|----------|
| **CURRENT** | [FG-021](feature-gates/FG-021-field-web-v1-today-and-capture.md) **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT OPEN**. Gate **NOT CLOSED**. Text **PASS**. Screenshot PNG **PASS**. **TAKE PHOTO PASS AS JPEG.** **HEIC REAL-DEVICE NOT YET TESTED.** **REAL IPHONE VOICE SAVE PASS** (Event **30**, `audio/mp4`). **REAL IPHONE NETWORK RETAIN / RETRY PASS** (Event **31**, text). Desktop continuity **PASS**. Observation Delete **QUEUED**. Approved presentation source custody **CLOSED**. Live current = head **`d2e3f4a5b6c7`**. Dedicated FG-021 **19**. Focused **147**. Full **557**. FG-008 through FG-020 **CLOSED / OPERATIONAL FOR UAT**. Native Signing: **development may proceed under separate governance**; **production activation blocked pending counsel process approval**. |
| **NEXT AUTHORIZED ACTION** | **STOP** for Joel/ChatGPT review. Remaining FG-021 real-iPhone closure UAT versus separately governed reusable-template extraction are **not chosen here**. Do **not** implement HEIC, mixed capture, Observation Delete, or FG-021 closure from this reconciliation. |
