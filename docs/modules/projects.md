...
- Change Orders lifecycle (draft → approval statuses) via `project_controls` package. **R01 (PKG-T01):** mutating Change Order / Punch / Walkthrough / Close services reload the Project (and related CO/estimate records) against the acting organization. Caller-loaded ORM identity is not tenant authority. **R04 (PKG-L01):** NEW operational mutations that call `raise_if_project_closed` claim `operating_state='ACTIVE'` in the same transaction as the business write; Close/Reopen use conditional UPDATEs. Concurrent Close vs NEW work fails closed (P7-03). Product SHA **`01a841e4c62d10264b97dc0bb20d636b5abb4d43`**. **R05 (PKG-L02):** public `submit_walkthrough_response` reuses that R04 claim in the same transaction as RESPONDED / item inserts. Token GET / expiry unchanged. Product SHA **`97f5f5a56c9e11e08a336698ed18bfdf57802528`**. **R03A (PKG-T03A):** contractor `accept_walkthrough_item_to_punch_list` is one transaction. Punch walkthrough-create participates with `commit=False`. Pending-item predicate contends with concurrent Accept. Standalone Punch create still commits. Product SHA **`424493b6286e8f59acba206e9c55fd410ced9318`**. **R03B (PKG-T03B):** combined Time Submit + Extra Work is one transaction. `create_extra_work(..., commit=False)` participates from `submit_time`. Extra-only Field/office create still commits. Time-only submit creates no Extra. Product SHA **`0ae5f1d12d06cbe16f12e6960c9849a98f0b397e`**. **R02 (PKG-T02, working tree / not committed):** Field `field_confirmed_project_id` may be written only for same-org ACTIVE Projects. Time GET still confirms ACTIVE without an extra confirm screen. CLOSED never becomes confirmed operating context. Today clears a stale CLOSED session id. R04 remains write-invariant authority.
- D1 Project Hub contextual Help **IMPLEMENTED** (`app/presentation/help_content.py`; native `<details>` on PLAN / PRICE / CONTRACT / BUILD / MONITOR; LEARN Future-only). D3 office Help **IMPLEMENTED** on the same authority (`topics_for_surface("office")`, `help_payload()`). D4 Field Help **IMPLEMENTED** on the same authority (`topics_for_surface("field")`, `field_topic()`). D5 Voice-with-Help **IMPLEMENTED** on the same authority (`answer_help_question()`, `POST /help/ask`). Voice does not mutate product state. No schema. Contextual Help coverage complete for Hub / Office / Field. User Guide remains outstanding. Subsequent **2026-09-21:** office `/` Home V2.2 is the current Calibrayt desktop reference (`app/services/home_planning.py`). Office `/projects/` VISUAL-2 V2 register is committed (`app/templates/projects/list.html`). Field Month and Field Projects unchanged.
...
- `tests/test_service_tenancy_invariant_r01.py` (R01 SERVICE TENANCY INVARIANT)
- `tests/test_closed_project_toctou_r04.py` (R04 CLOSED-PROJECT TOCTOU)
- `tests/test_public_walkthrough_submit_on_closed_r05.py` (R05 PUBLIC-WALKTHROUGH-SUBMIT-ON-CLOSED)
- `tests/test_walkthrough_accept_atomicity_r03a.py` (R03A BUSINESS-ACTION-ATOMICITY — Walkthrough Accept)
- `tests/test_time_extra_atomicity_r03b.py` (R03B BUSINESS-ACTION-ATOMICITY — Time + Extra combined)
- `tests/test_field_project_context_r02.py` (R02 SESSION-CONTEXT ISOLATION — Field Project)
