...
| Documentation status | FG-013 **IMPLEMENTED / VERIFIED · LIVE MIGRATION PENDING**. ADR-032 **Accepted**. ADR-021 **Accepted** (docs only). FG-012 **CLOSED / OPERATIONAL FOR UAT**. ADR-010 **Proposed**. |
...
| Decisions pending | FG-013 **live migrate + UAT**. Real AI provider; Phase D mapping gate. FG-009 carry-forward unchanged. |
...
| Approved next Cursor prompt location or summary | Separate **live-migrate** prompt for `c5d6e7f8a9b0`. Do not implement MONITOR. Do not start Phase D. Do not enable external AI. |
