...
| Attribute | Value |
|-----------|--------|
| Status | **FG-021 IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT OPEN.** Gate **NOT CLOSED**. Text-only **PASS**. Small screenshot PNG **PASS** (Event **27**). **TAKE PHOTO PASS AS JPEG** (Event **28** / Original **27**, `image/jpeg`). **HEIC REAL-DEVICE NOT YET TESTED.** Voice UAT **BLOCKED** on HTTP LAN. **LOCAL HTTPS UAT SETUP STARTED** (certs outside git; HTTP **5014** preserved; HTTPS listening on **5443**). Voice **NOT** claimed PASS. iPhone trust **NOT** complete. Live still **28** Events / **27** Originals; no new Project 11 audio Original. Observation Delete **QUEUED**. Live current = head **`d2e3f4a5b6c7`**. Do **not** close FG-021. |
| Updated | 2026-09-02 |
...
**Last authorized delta:** FG-021 **LOCAL HTTPS UAT SETUP STARTED** (voice enablement environment only; no product-code change). OpenSSL LibreSSL local CA + leaf written **outside git** at `/Users/joelbrayman/Desktop/CalibAi-UAT-TLS/` (public iPhone CA `CalibAi-UAT-Local-CA.cer`). Leaf SAN: `DNS:Joels-MacBook-Air.local`, `IP:192.168.134.223`. HTTP **5014** Flask **562293** left running. Separate HTTPS Flask listening on **5443** (`https://Joels-MacBook-Air.local:5443`). IndexedDB is origin-scoped — HTTP `http://192.168.134.223:5014` and this HTTPS origin are different; no voice pending was created on HTTP; **do not migrate**. Voice **NOT** claimed PASS. iPhone trust **NOT** complete. Gate **NOT CLOSED**.
...
**Next governed action:** Get the public CA file `/Users/joelbrayman/Desktop/CalibAi-UAT-TLS/CalibAi-UAT-Local-CA.cer` onto the iPhone (AirDrop). Do **not** browse the HTTPS URL yet. Do **not** start voice. Do **not** claim iPhone trust complete. Do **not** close [FG-021](feature-gates/FG-021-field-web-v1-today-and-capture.md). Do **not** implement PWA, audio conversion, mixed capture, HEIC, network retry, or Observation Delete from this setup. Native Signing **development** may proceed only under **separate** governance; **production activation** remains blocked pending Ontario counsel process approval.
