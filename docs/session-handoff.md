...
- **FG-021:** **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT OPEN**. [ADR-043](adr/ADR-043-field-web-capture-reliability-local-pending-and-idempotent-replay.md) **Accepted**. Live current = head `d2e3f4a5b6c7`. Gate **NOT CLOSED**. iPhone Safari text-only **PASS** (Event **18** / Original **18**). UUID repair + text path **PASS**. Photo / IndexedDB quota **still open**. Real iPhone UAT **not complete**. Dedicated **15**. Focused **143**.
...
**Next governed action:** Desktop Event **18** continuity **PASS**. One iPhone diagnostic only: **CHOOSE PHOTO** (small screenshot or known JPEG from the library, then Save) on the same HTTP LAN capture URL. Do **not** TAKE PHOTO. Do **not** attach a full-size HEIC. Do **not** implement File→Blob until a verified CASE B. Do **not** close [FG-021](feature-gates/FG-021-field-web-v1-today-and-capture.md). Native Signing **development** may proceed only under **separate** governance; **production activation** remains blocked pending Ontario counsel process approval. Do **not** implement Native Signing production, mail, Closeout, or Contract templates from this pass.
