...
**Subsequent status (2026-09-23 PKG-S16 working tree):** R24 wage-event persistence **`organization_person_wage_events`** is **IMPLEMENTED IN WORKING TREE / NOT LIVE-MIGRATED / ZERO SYNTHETIC BACKFILL**. Additive **`h8c9d0e1f2a3`**. Live Alembic remains **`g7b8c9d0e1f2 (head)`**. Current `OrganizationPerson.hourly_wage` is unchanged. `create_person` / `update_person` / CLI are **not** wired. PA-D **NOT IMPLEMENTED**. This freeze is **not rewritten**. People & Access UI remains **NOT IMPLEMENTED**.
