...
| Layer | Position |
|-------|----------|
| **CURRENT** | [FG-021](feature-gates/FG-021-field-web-v1-today-and-capture.md) **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT OPEN**. Gate **NOT CLOSED**. Text **PASS**. Screenshot PNG **PASS**. **TAKE PHOTO PASS AS JPEG.** **HEIC REAL-DEVICE PASS** (Event **34** / Original **32**, Files/Browse `IMG_5351.HEIC`). **MIXED CAPTURE PASS** (Event **35**, Originals **33** text / **34** audio / **35** JPEG). **BACKGROUND / FOREGROUND PERSISTENCE PASS** (Event **36** / Original **36**, text). **CSRF RECOVERY PASS** (Event **39** / Original **39**, text). Portrait **PASS**. One-handed **PASS**. Outdoor readability **PASS**. **LANDSCAPE TOLERANCE: DEFECT CORRECTED IN CODE / REAL-IPHONE RETEST REQUIRED / NOT YET PASS.** **REAL IPHONE VOICE SAVE PASS** (Event **30**, `audio/mp4`). **REAL IPHONE NETWORK RETAIN / RETRY PASS** (Event **31**, text). **REAL IPHONE BROWSER CLOSE → INDEXEDDB RECOVERY → RETRY PASS** (Event **32**, JPEG). Desktop continuity **PASS**. Observation Delete **QUEUED**. Approved presentation source custody **CLOSED**. [FG-022](feature-gates/FG-022-reusable-approved-document-template-family-v1.md) **CLOSED / APPROVED REUSABLE MASTER FAMILY V1**. Live current = head **`d2e3f4a5b6c7`**. Dedicated FG-021 **20**. Focused **148**. Full **558**. FG-008 through FG-020 **CLOSED / OPERATIONAL FOR UAT**. Native Signing: **development may proceed under separate governance**; **production activation blocked pending counsel process approval**. |
| **NEXT AUTHORIZED ACTION** | **STOP.** Do **not** close FG-021. Landscape-tolerance CSS is implemented. Real-iPhone landscape retest is required before PASS. Do **not** start older-device or session-expiry UAT from this pass. Do **not** implement Observation Delete from this pass. |
