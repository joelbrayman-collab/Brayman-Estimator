...
- **Approved, next:** **STOP.** [ADR-042](adr/ADR-042-build-field-evidence-and-iphone-first-capture.md) is **Proposed / FOR JOEL REVIEW**. Do not accept it in an implementation prompt. Do not create FG-020. Do not start BUILD, Field Web, tokens, RBAC, or org-switcher. Item 11 is **ELIGIBLE FOR SEPARATE GOVERNANCE / NOT AUTHORIZED**.
- **Roadmap direction (not authorization):** item 10 is **COMPLETE**. Item 11 BUILD architecture is drafted as ADR-042 **Proposed**. **ROADMAP SEQUENCE ≠ IMPLEMENTATION AUTHORIZATION.**
