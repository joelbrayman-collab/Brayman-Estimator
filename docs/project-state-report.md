...
| Current branch | `main` |
| Current commit / `origin/main` | FG-017 live-migrate / office UAT close (this commit). Implementation parent `00ca492e28118d75757e9a9c82384978b5decd92`. FG-016 close `fa591f14b2eb99db75c4e3720fdeb30d14a8f77a`. |
...
| Decisions made (this FG-017 live-migrate/UAT pass) | Applied `f8a9b0c1d2e3` → `a9b0c1d2e3f4`. Ensure Brand Profiles for 2 orgs. Backfill 0 snapshots (no prior Issued/Accepted). Office UAT port **5010**. Issued freeze + post-change isolation verified. No product-code change. Office UAT only — not broader production validation. |
...
| Uncommitted work | None after FG-017 live-migrate docs close. |
| Next approved milestone | **STOP.** [FG-017](feature-gates/FG-017-organization-brand-profile-v1.md) is **CLOSED / OPERATIONAL FOR UAT**. Do not start the next Feature Gate. |
| Next candidate milestone | Phase D, supplier onboarding, Change Order documents, MONITOR, and national permit expansion remain separately governed and unauthorized. |
| Documents to read first | [session-handoff.md](session-handoff.md) → [current-state.md](current-state.md) → [project-state-report.md](project-state-report.md) → [feature-gates/FG-017-organization-brand-profile-v1.md](feature-gates/FG-017-organization-brand-profile-v1.md) |
| Approved next Cursor prompt location or summary | **STOP.** Fresh-chat prompt in [session-handoff.md](session-handoff.md) §22. |
| Commit status | FG-017 live-migrate / office UAT close (this commit) on `main`. Implementation parent `00ca492e28118d75757e9a9c82384978b5decd92`. Live current = head `a9b0c1d2e3f4`. FG-016 **CLOSED / OPERATIONAL FOR UAT**. ADR-040 **Accepted**. FG-017 **CLOSED / OPERATIONAL FOR UAT**. |
