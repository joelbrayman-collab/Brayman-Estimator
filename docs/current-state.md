...
|-------|--------|
| Branch | `main` |
| HEAD / `origin/main` | FG-020 implementation: verify `git rev-parse HEAD` and `git rev-parse origin/main` after this commit. Live Alembic current **`b0c1d2e3f4a5`**. Repository Alembic head **`c1d2e3f4a5b6`**. |
...
| **Working tree at last verified inspect** | **FG-020 IMPLEMENTED / LIVE MIGRATION PENDING.** Live current `b0c1d2e3f4a5`. Repository head `c1d2e3f4a5b6`. Full suite **527 passed**. Dedicated FG-020 **33 passed**. Focused **370 passed**. [ADR-042](adr/ADR-042-build-field-evidence-and-iphone-first-capture.md) **Accepted**. FG-008–FG-019 **CLOSED / OPERATIONAL FOR UAT**. Item 12 **BLOCKED / NOT AUTHORIZED**. Do **not** mark FG-020 closed until live migration + office UAT. |
...
- **BUILD Field Capture V1 (FG-020)** — **IMPLEMENTED / LIVE MIGRATION PENDING.** Field Capture Events, Original Payloads (text/audio/image including HEIC/HEIF originals without transcode), Derived Candidates, private custody `instance/build_originals/`, office Field Observations beside Change Orders, bounded `/api/v1` BUILD routes, `flask build propose-derived-candidate`. Additive revision **`c1d2e3f4a5b6`** is the repository head; live current remains **`b0c1d2e3f4a5`**. Dedicated tests **33 passed**. Full suite **527 passed**. Not closed. Field Web **not** implemented. Live `flask db upgrade` **not** run.
...
- **Approved, next:** Separate **live migration** (`b0c1d2e3f4a5` → `c1d2e3f4a5b6`) then office UAT. Do **not** mark FG-020 **CLOSED / OPERATIONAL FOR UAT** yet. Do **not** start Field Web.
- **Roadmap direction (not authorization):** item 10 is **COMPLETE**. Item 11 is implemented / live migration pending. **ROADMAP SEQUENCE ≠ IMPLEMENTATION AUTHORIZATION.**
- **Blocked / Not Started (product):** Field Web; Phase D estimate mapping; four-output package outputs 3–4; QuickBooks; contracts; MONITOR implementation; LEARN; industry benchmarking; historical evidence repair; real external AI provider; supplier/Winchester POC; bulk supplier onboarding; national Permit Rules expansion; Change Order document family; Permit branding; RBAC; org-switcher.
...
1. **STOP after this implementation.** [FG-020](feature-gates/FG-020-build-field-capture-v1-project-field-observation-foundation.md) is **IMPLEMENTED / LIVE MIGRATION PENDING**. Live current remains `b0c1d2e3f4a5`. Repository head is `c1d2e3f4a5b6`. Do **not** run live `flask db upgrade` until a separate live-migration prompt. Do **not** mark FG-020 **CLOSED / OPERATIONAL FOR UAT**. Do **not** start Field Web. Do not begin national permit expansion, Phase D, Change Order document work, supplier integration, or external AI / runtime web. Do not accept ADR-008. Do not accept ADR-010.
2. **Next authorized prompt:** live-migrate `b0c1d2e3f4a5` → `c1d2e3f4a5b6`, then office UAT of Field Observations beside Change Orders. Item 12 remains **BLOCKED / NOT AUTHORIZED**.
