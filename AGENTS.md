...
| Updated | 2026-09-04 |
| Product | The Estimator / CalibraytAI |
...
- Follow the **chat-title continuity convention** in that protocol: begin every CalibraytAI development response with the exact ChatGPT development chat title in bold; end with `END — <exact title>` **after** any Cursor prompt. Cursor/IDE workspace titles remain `BRAYMAN — <Topic>`. Every CalibraytAI development response ends with the next complete ready-to-paste Cursor prompt unless Joel explicitly says no prompt is required.
