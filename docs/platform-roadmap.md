...
| Layer | Position |
|-------|----------|
| **CURRENT** | [FG-021](feature-gates/FG-021-field-web-v1-today-and-capture.md) **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT OPEN**. Gate **NOT CLOSED**. Text-only **PASS**. Desktop continuity **PASS**. IndexedDB photo-put repair **LANDED**. Small-JPEG re-UAT **pending**. Observation Delete **QUEUED / NOT AUTHORIZED**. Live current = head **`d2e3f4a5b6c7`**. Dedicated FG-021 **17**. Focused **145**. Full **555**. FG-008 through FG-020 **CLOSED / OPERATIONAL FOR UAT**. Native Signing: **development may proceed under separate governance**; **production activation blocked pending counsel process approval**. |
| **NEXT AUTHORIZED ACTION** | iPhone Safari **CHOOSE PHOTO** one small screenshot/JPEG then Save (hard-refresh `field.js` first). Do **not** implement Observation Delete. Do **not** close FG-021. Native Signing **production** remains blocked pending counsel. Do **not** implement Project Closeout or Contract templates. |
...
| **ROADMAP DIRECTION** | Item 11 is **COMPLETE**. Item 12 Field Web is **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT PENDING**. [ADR-043](adr/ADR-043-field-web-capture-reliability-local-pending-and-idempotent-replay.md) **Accepted**. **ROADMAP SEQUENCE ≠ IMPLEMENTATION AUTHORIZATION.** |
| **SEPARATELY GOVERNED FUTURE PROGRAMS** | Project Closeout / archive-and-purge; Native Signing production; Phase D; supplier integration; Change Order document family; Permit branding / national expansion; QuickBooks; Ontario Contract / Warranty; MONITOR; LEARN; real external AI; other repository-recorded future work. Not reordered here. Not authorized by this increment. |
...
12. Field Web / Today + Capture + plan access — **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT PENDING** ([FG-021](feature-gates/FG-021-field-web-v1-today-and-capture.md)). Gate **NOT CLOSED**. Plan access deferred to a later Field slice. Live current = head `d2e3f4a5b6c7`.
...
1. **FG-021 IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT OPEN.** Gate **NOT CLOSED**. Live current = head `d2e3f4a5b6c7`. Text-only iPhone path **PASS**. IndexedDB photo-put repair **LANDED**. Next: small-JPEG CHOOSE PHOTO re-UAT. Observation Delete **queued** ([capture](architecture/fg-021-recent-observation-delete-requirement-capture.md)) — do **not** implement yet. Close only after remaining real iPhone UAT under a later prompt. Do **not** implement Project Closeout. Do not implement MONITOR, Phase D, supplier/Winchester POC, bulk supplier onboarding, national permit expansion, or Change Order documents. Do not accept ADR-008. Do not accept ADR-010.
2. **Roadmap direction:** Item 12 Field Web is **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT PENDING**. Native Signing **development** may proceed under separate governance; **production activation** remains blocked pending counsel.
...
- BUILD Field Observation foundation — **CLOSED / OPERATIONAL FOR UAT** ([FG-020](feature-gates/FG-020-build-field-capture-v1-project-field-observation-foundation.md)); Field Web (sequence item 12) **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT PENDING** ([FG-021](feature-gates/FG-021-field-web-v1-today-and-capture.md))
