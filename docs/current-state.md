...
|-------|--------|
| Branch | `main` |
| HEAD / `origin/main` | FG-021 live-migration docs: verify `git rev-parse HEAD` and `git rev-parse origin/main` after this commit. Implementation parent `5c36f6fcdf3c54aab9d103cd5152685382618984`. Live Alembic current = head **`d2e3f4a5b6c7`**. |
...
- **Field Web V1 Today + Capture (FG-021)** — **IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT PENDING.** `/field` Today, Project confirm, Capture (photo/voice/text). IndexedDB `calibai-field-v1`. Idempotent Event/Original POST (201/200/409). Display GET for Field image preview. Additive revision **`d2e3f4a5b6c7` applied live** (`c1d2e3f4a5b6` → `d2e3f4a5b6c7`). Live current = head. Gate **NOT CLOSED**. Real iPhone UAT **not complete**. No PWA. No transcription. Dedicated tests **13 passed**. Focused **141 passed**. Full suite **551 passed**.
...
1. **FG-021 IMPLEMENTED / LIVE-MIGRATED / IPHONE UAT PENDING.** Gate **NOT CLOSED**. Live current = head `d2e3f4a5b6c7`. Do **not** close FG-021 until real iPhone Safari UAT passes. Do not begin Native Signing production, Closeout, MONITOR, transcription, PWA, or external AI. Do not accept ADR-008. Do not accept ADR-010.
