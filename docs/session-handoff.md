...
- FG-035 CORE CLOSE Slice A: `Project.operating_state` / `operating_state_changed_at` / `operating_state_changed_by_user_id`; `ProjectOperatingStateEvent`; `list_current_operating_projects` in `app/services/shared_api.py`; additive **`b2c3d4e5f6a7` applied live** 2026-09-18. Product SHA **`f4b7515664c51850f3d87ed79f4a7e1226886fbd`**. No `close_project` / `reopen_project`. Live consumers still call `list_organization_projects`.
