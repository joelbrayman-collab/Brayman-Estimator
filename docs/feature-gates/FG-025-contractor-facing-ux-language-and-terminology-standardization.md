...
**Subsequent status (2026-09-16 platform-wide desktop Print / paper workflow recording):** Print availability and print quality are added to the already-mandatory Contractor Language + UX E2E Audit ([architecture/project-element-authority-future-record.md](../architecture/project-element-authority-future-record.md)). It does **not** reopen this gate, does **not** close this gate, and does **not** authorize another FG-025 slice or Print product.
...
SLICE 3 IMPLEMENTED / TESTED / COMMITTED / PUSHED
SLICE 4 IMPLEMENTED / TESTED / COMMITTED / PUSHED
SLICE 5 IMPLEMENTED / TESTED / COMMITTED / PUSHED
NOT CLOSED
NOT YET PRODUCT-WIDE COMPLETE
...
Office PRICE specialist copy:

- Pricing office title **Pricing**. Method `TRUE_GROSS_MARGIN` displays **Gross Margin Pricing** (never Markup Pricing; internal key unchanged). `COST_PLUS_MARKUP` displays **Cost plus markup**. `COST_PLUS_MARKUP_STACK` displays **Cost plus markup (legacy stack)**.
- Labour office title **Labour rates** (org rates/tasks/standards; not a job labour estimate). Hub PRICE links: **Pricing** / **Labour rates**.
- Estimate office: **Estimate version**, **Pricing lock**; Estimate ≠ Proposal preserved.
...
## SLICE 4 IMPLEMENTATION (2026-09-08)

**Status:** **SLICE 4 IMPLEMENTED / TESTED / COMMITTED / PUSHED.** Gate **NOT CLOSED.** Remaining FG-025 surfaces **NOT AUTHORIZED.** Not a product-wide sweep.

Presentation-layer mapping only, reusing `app/presentation/contractor_copy.py`. No competing copy system. No schema, migration, live DB write, pricing/labour/estimate arithmetic change, enum/model rename, MONITOR change, Field Web rewrite, customer PDF rewrite, LEARN, or FG-024.

Shared office shell copy:

- Global nav **Labour Engine** → **Labour rates**; **Pricing Engine** → **Pricing**. Endpoints remain `labour_engine.index` / `pricing_engine.index`.
- Dashboard heading **Office home**; lede tells the contractor what to do next. Estimate/proposal activity status uses `office_status_label()`.
...
## SLICE 5 IMPLEMENTATION (2026-09-08)

**Status:** **SLICE 5 IMPLEMENTED / TESTED / COMMITTED / PUSHED.** Gate **NOT CLOSED.** Remaining FG-025 surfaces **NOT AUTHORIZED.** Not a product-wide sweep.

Presentation-layer mapping only, reusing `app/presentation/contractor_copy.py`. No competing copy system. No schema, migration, live DB write, Field workflow change, media pipeline change, enum/model rename, Observation Delete, session-revocation implementation, office UI rewrite, customer PDF rewrite, MONITOR change, LEARN, or FG-024.

Field Web user-facing copy (`app/templates/field/*.html`, `app/static/js/field.js` visible strings):

- Header **Log out** → **Sign out**. Logout route, CSRF, and session behavior unchanged.
- Today **Switch Project** → **Change project**. Retry heading **Not sent yet**; retry link **Try sending again**.
- Capture notes label **Notes** (was Short text). **Save original** preserved for provenance.
- Visible save status **Saved** / **Saving…** / **Could not send** (internal `saved` / `saving` / `needs_retry` state keys unchanged).
...
- Unauthorized candidate surfaces: customer Proposal/PDF terminology; Historical Evidence nav and historical specialist screens; standalone Permit screens; Hub PRICE table leftover `TRUE_GROSS_MARGIN`; final product-wide terminology sweep.
- **REVIEW / DECISION REQUIRED** (not implied must-change): Cost Items nav label; “Contract value” terminology; Dashboard `page_title` still Dashboard; Office sign in / Brayman Construction Platform titles retained; `GENERIC_LOGIN_FAILURE` retained for security; Brand Profile saved. flash retained; disabled Settings (coming soon) frozen by FG-017.

Do **not** start another FG-025 slice from this section.
